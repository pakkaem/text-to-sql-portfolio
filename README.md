...
1. **Layanan AI (Python/FastAPI):** Bertindak sebagai "otak" sistem. Memuat base model **TinyLlama-1.1B** yang telah di-*fine-tune* menggunakan metode **LoRA (Low-Rank Adaptation)** untuk secara spesifik menghasilkan *query* SQL yang valid berdasarkan konteks skema *database*.
2. **Layanan Backend (Golang/Gin):** Bertindak sebagai "jembatan" utama. Menerima *request* HTTP dari pengguna, menyuntikkan skema DDL, meminta *query* SQL dari layanan AI, dan mengeksekusi *query* tersebut secara langsung ke *database* menggunakan teknik *Dynamic SQL Scanning*.
3. **Database (SQLite):** Penyimpanan data terintegrasi (No-Ops) yang dikonfigurasi untuk terbuat otomatis beserta *dummy data* saat aplikasi pertama kali dijalankan.
